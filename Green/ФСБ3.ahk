...
FileReadLine, line, %way%, strings
{
    ...
    if line contains нейтрализовал преступника
    {
	if (InStr(line, CRMP_USER_NICKNAME) or InStr(line, Mask))
    	{
            now := A_TickCount
...
:?:!вгп::
SendMessage, 0x50,, 0x4190419,, A
SendInput /d [%TegText%] Говорит %Teg%. Стройте состав на парковке УФСБ, готовность -  минут.{left 7}
Return

:?:!вгпк::
SendMessage, 0x50,, 0x4190419,, A
SendInput /d [%TegText%] Говорит %Teg%. Спец. операция «Крушение ВГП» окончена. Снимаем оцепление, едем по должностным.{left 1}
Return
...
:?:!стоп::
SetTimer, AutoReportTick, Off
if (RemainingTime > 0 && SelectedReportType != "" && SelectedOrganization != "")
{
    if (SelectedReportType="прослушка")
        sendChat("/r Докладывает " . Teg . ": досрочно завершил прослушивание организации «" . SelectedOrganization . "».")
    else if (SelectedReportType="надзор")
        sendChat("/r Докладывает " . Teg . ": досрочно завершил надзор за организацией «" . SelectedOrganization . "».")
    else if (SelectedReportType="внедрение")
        sendChat("/r Докладывает " . Teg . ": досрочно завершил внедрение в организацию «" . SelectedOrganization . "».")
    ...
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Авто-доклад был досрочно завершён.")
}
else
{
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} В данный момент нет активного авто-доклада.")
}
return
...
SelectedReportType := ""
SelectedOrganization := ""
toggleUserHotkeys("Off")
addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Выберите тип доклада:")
addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Нажмите {58e11b}1{FFFFFF} - Доклады о прослушке")
addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Нажмите {58e11b}2{FFFFFF} - Доклады о надзорной деятельности")
addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Нажмите {58e11b}3{FFFFFF} - Доклады о внедрении")
addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Нажмите {58e11b}0{FFFFFF} - Отмена")
SetTimer, DeactivateMenu, -15000
disableAllHotkeys()
Hotkey, 1, MenuType1, On
Hotkey, 2, MenuType2, On
Hotkey, 3, MenuType3, On
Hotkey, 0, CancelMenu, On
return

MenuType1:
MenuType2:
MenuType3:
if (!MenuActive || CurrentMenuLevel != 1)
    return
SetTimer, DeactivateMenu, Off
disableAllHotkeys()
if (A_ThisLabel = "MenuType1")
    SelectedReportType := "прослушка"
else if (A_ThisLabel = "MenuType2")
    SelectedReportType := "надзор"
else if (A_ThisLabel = "MenuType3")
    SelectedReportType := "внедрение"
SetTimer, ShowOrgMenu, -30
return

ShowOrgMenu:
ShowOrganizationMenu(SelectedReportType)
return

ShowOrganizationMenu(rType)
{
    global MenuActive, CurrentMenuLevel
    MenuActive := true
    CurrentMenuLevel := 2
    disableAllHotkeys()
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Выберите организацию:")
    if (rType = "прослушка")
    {
        addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} {58e11b}1{FFFFFF}-Правительство | {58e11b}2{FFFFFF}-УГИБДД | {58e11b}3{FFFFFF}-УМВД")
        addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} {58e11b}4{FFFFFF}-ВЧ | {58e11b}5{FFFFFF}-ЕСС | {58e11b}6{FFFFFF}-ТРК")
        addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} {58e11b}7{FFFFFF}-Скинхеды | {58e11b}8{FFFFFF}-Байкеры | {58e11b}9{FFFFFF}-Кавказская мафия")
        addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} {58e11b}Alt+1{FFFFFF}-Русская мафия | {58e11b}Alt+2{FFFFFF}-ФСИН | {58e11b}0{FFFFFF}-Отмена")
        Hotkey, 1, OrgGov, On
        Hotkey, 2, OrgGIBDD, On
        Hotkey, 3, OrgUMVD, On
        Hotkey, 4, OrgVC, On
        Hotkey, 5, OrgESS, On
        Hotkey, 6, OrgTRK, On
        Hotkey, 7, OrgSkin, On
        Hotkey, 8, OrgBike, On
        Hotkey, 9, OrgKavkaz, On
        Hotkey, LAlt & 1, OrgRus, On
        Hotkey, RAlt & 1, OrgRus, On
        Hotkey, LAlt & 2, OrgFSIN, On
        Hotkey, RAlt & 2, OrgFSIN, On
    }
    else if (rType = "надзор")
    {
        addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} {58e11b}1{FFFFFF}-Правительство | {58e11b}2{FFFFFF}-УГИБДД | {58e11b}3{FFFFFF}-УМВД")
        addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} {58e11b}4{FFFFFF}-ВЧ | {58e11b}5{FFFFFF}-ЕСС | {58e11b}6{FFFFFF}-ФСИН | {58e11b}0{FFFFFF}-Отмена")
        Hotkey, 1, OrgGov, On
        Hotkey, 2, OrgGIBDD, On
        Hotkey, 3, OrgUMVD, On
        Hotkey, 4, OrgVC, On
    ...
    else if (rType = "внедрение")
    {
        addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} {58e11b}1{FFFFFF}-УГИБДД | {58e11b}2{FFFFFF}-УМВД | {58e11b}3{FFFFFF}-ВЧ")
        addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} {58e11b}4{FFFFFF}-ЕСС | {58e11b}5{FFFFFF}-ТРК | {58e11b}6{FFFFFF}-Скинхеды")
        addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} {58e11b}7{FFFFFF}-Байкеры | {58e11b}8{FFFFFF}-Кавказская мафия | {58e11b}9{FFFFFF}-Русская мафия")
        addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} {58e11b}Alt+1{FFFFFF}-ФСИН | {58e11b}0{FFFFFF}-Отмена")
        Hotkey, 1, OrgGIBDD, On
        Hotkey, 2, OrgUMVD, On
        Hotkey, 3, OrgVC, On
        Hotkey, 4, OrgESS, On
        Hotkey, 5, OrgTRK, On
        Hotkey, 6, OrgSkin, On
        Hotkey, 7, OrgBike, On
        Hotkey, 8, OrgKavkaz, On
        Hotkey, 9, OrgRus, On
        Hotkey, LAlt & 1, OrgFSIN, On
        Hotkey, RAlt & 1, OrgFSIN, On
    }
    Hotkey, 0, CancelMenu, On
    SetTimer, DeactivateMenu, -15000
}
...
OrgRus:
OrgFSIN:
if (!MenuActive || CurrentMenuLevel != 2)
    return
SetTimer, DeactivateMenu, Off
disableAllHotkeys()
if (A_ThisLabel = "OrgGov")
    SelectedOrganization := "Правительство"
else if (A_ThisLabel = "OrgGIBDD")
    SelectedOrganization := "УГИБДД"
else if (A_ThisLabel = "OrgUMVD")
    SelectedOrganization := "УМВД"
else if (A_ThisLabel = "OrgVC")
    SelectedOrganization := "ВЧ"
else if (A_ThisLabel = "OrgESS")
    SelectedOrganization := "ЕСС"
else if (A_ThisLabel = "OrgTRK")
    SelectedOrganization := "ТРК"
else if (A_ThisLabel = "OrgSkin")
    SelectedOrganization := "Скинхеды"
else if (A_ThisLabel = "OrgBike")
    SelectedOrganization := "Байкеры"
else if (A_ThisLabel = "OrgKavkaz")
    SelectedOrganization := "Кавказская мафия"
else if (A_ThisLabel = "OrgRus")
    SelectedOrganization := "Русская мафия"
else if (A_ThisLabel = "OrgFSIN")
    SelectedOrganization := "ФСИН"
MenuActive := false
CurrentMenuLevel := 0
disableMenuHotkeys()
toggleUserHotkeys("On")
Goto, AutoStartReport
return

AutoStartReport:
InputBox, ReportTime, Авто-доклад, Введите общее время доклада (в минутах, кратно 10):, , 260, 140
if (ErrorLevel)
{
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Авто-доклад отменён.")
    return
}
if (Mod(ReportTime,10)!=0)
{
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Введите время, кратное 10 (например, 10, 20, 30...).")
    return
}
TotalTime := ReportTime*60000
Interval := 10*60000
RemainingTime := TotalTime

if (SelectedReportType="прослушка")
    sendChat("/r Докладывает " . Teg . ": начал прослушивание организации «" . SelectedOrganization . "».")
else if (SelectedReportType="надзор")
    sendChat("/r Докладывает " . Teg . ": начал проводить надзор за организацией «" . SelectedOrganization . "».")
else if (SelectedReportType="внедрение")
    sendChat("/r Докладывает " . Teg . ": начал внедрение в организацию «" . SelectedOrganization . "».")
TakeScreenshot()

SetTimer, AutoReportTick, %Interval%
addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Авто-доклад активирован на " . ReportTime . " минут, каждые 10 минут доклад о продолжении.")
addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Для досрочной отмены используйте команду {58e11b}!стоп")
return

AutoReportTick:
global RemainingTime, Interval, SelectedReportType, SelectedOrganization, Teg
RemainingTime -= Interval
if (RemainingTime <= 0)
{
    sendChat("/r Докладывает " . Teg . ": закончил " . (SelectedReportType="прослушка"?"прослушивание организации":SelectedReportType="надзор"?"надзор за организацией":"внедрение в организацию") . " «" . SelectedOrganization . "».")
    TakeScreenshot()
    SetTimer, AutoReportTick, Off
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Авто-доклад завершён.")
    disableMenuHotkeys()
    return
}
sendChat("/r Докладывает " . Teg . ": продолжаю " . (SelectedReportType="прослушка"?"прослушивание организации":SelectedReportType="надзор"?"надзор за организацией":"внедрение в организацию") . " «" . SelectedOrganization . "».")
TakeScreenshot()
return
...
disableAllHotkeys()
{
    global KeyCar, KeyCars, Rozisk, KeyOne, KeyThree, KeyFour, KeyFive, KeySix, KeySeven, KeyEight, KeyNine
    global KeyPolice, KeyVesti, KeyPosadi, KeyProtokol, KeyProbiv, vivs, ProbivGos, ProbivGosId
    global 1help, 2help, 3help
    
    arr := [KeyCar, KeyCars, Rozisk, KeyOne, KeyThree, KeyFour, KeyFive, KeySix, KeySeven, KeyEight, KeyNine
        , KeyPolice, KeyVesti, KeyPosadi, KeyProtokol, KeyProbiv, vivs, ProbivGos, ProbivGosId
        , 1help, 2help, 3help]
    for i, hk in arr
        if (hk != "")
    ...
    Hotkey, LAlt & 1, Off, UseErrorLevel
    Hotkey, RAlt & 1, Off, UseErrorLevel
    Hotkey, LAlt & 2, Off, UseErrorLevel
    Hotkey, RAlt & 2, Off, UseErrorLevel
}
