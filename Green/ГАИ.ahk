...
KeyFive:
SendChat("На основании статьи 22 процессуального кодекса, Вам предоставляются следующие права:")
Sleep 1000
...
KeyNine:
sendChat("/m [" . TegText . "] Принимаем правое положение и делаем остановку!")
sleep 600
...
addChatMessageEx(0, "{58e11b}[Диспетчер]{FFFFFF} В случае изменения обычных клавиш в подсказке на иные обращайтесь так же в вк.")
addChatMessageEx(0, "          ")
}
LABEL_RELOAD:
Reload
...
Sleep 1000
SendChat("/me правой рукой берёт ручку, затем заполняет в бланке данные задержанного, свои данные, место, время, мотивы, причины и обстоятельства задержания")
Sleep 1000
...
:?:!кор::
SendMessage, 0x50,, 0x4190419,, A
SendInput /d [ГАИ] Ввожу корректировки в ЛД № по статье 36 ПК.{left 17}
Return

:?:!док::
sendChat("/do В нагрудном кармане удостоверение.")
Sleep 500
sendChat("/me достал из нагрудного кармана удостоверение, закрыв фотокарточку и ФИО, показал его не передавая в руки на уровне глаз")
Sleep 500
sendChat("/me ..затем, после ознакомления, свернул удостоверение и убрал обратно в нагрудный карман")
Sleep 500
sendChat("/doc " playerID)
Return
...
vivs:
sendChat("/me демонстрирует протокол задержания на уровне глаз задержанного")
Sleep 1000
...
Sleep 1000
sendChat("/do Дежурный ушёл с задержанным.")
Sleep 1000
sendChat("/frac " playerID)
sleep 450
SendInput, {1}{1}
...
:?:!вн::
SendMessage, 0x50,, 0x4190419,, A
SendInput /d [%TegText%] %Rank% %Teg% на связи. Вызов срочной помощи внутренний.{left 1}
Return
...
:?:!поме::
SendMessage, 0x50,, 0x4190419,, A
SendInput /d [%TegText%] %Rank% %Teg% на связи. Помехи... {left 1}
Return

:?:!всеобщее::
SendMessage, 0x50,, 0x4190419,, A
SendInput /r Всеобщее построение на парковке УГИБДД{!} Готовность -  минут. Неявка - выговор с занесением в личное дело по 8.1 ВУ ОВД.{left 66}
Return

:?:!ор::
SendMessage, 0x50,, 0x4190419,, A
SendInput /r Построение ОР на парковке УГИБДД{!} Готовность -  минут. Неявка - выговор с занесением в личное дело по 8.1 ВУ ОВД.{left 66}
Return

:?:!орлс::
SendMessage, 0x50,, 0x4190419,, A
SendInput /r Построение ОРЛС на парковке УГИБДД{!} Готовность -  минут. Неявка - выговор с занесением в личное дело по 8.1 ВУ ОВД.{left 66}
Return
...
CreateWindow5() {
    ...
    Gui, 5:Color, black
    Gui, 5:Font, s7 cWhite w%Speed1%
    
    If UserID {
        Gui, 5:Add, Text,, ID = %UserID%
    }
    
    Gui, 5:Add, Text,,     [Код]         			[Значение]
    Gui, 5:Add, Text,,       1-1				Построение на СП
    Gui, 5:Add, Text,,       1-2/1-3				Сбор в лекционной в масках/без
    Gui, 5:Add, Text,,       1-4				Выезд СР по координатам
    Gui, 5:Add, Text,,       1-5				Построение состава по координатам
    Gui, 5:Add, Text,,       2-1/2-2/2-3			Н/П/К стоять на посту
    Gui, 5:Add, Text,,       2-4/2-5/2-6			Н/П/К патрулирования
    Gui, 5:Add, Text,,       2-7/2-8				Н/К отработки SOS
    Gui, 5:Add, Text,,       2-11/2-12/2-13			Н/П/К сопровождения ВК
    Gui, 5:Add, Text,,       2-14				Участие в СО
    Gui, 5:Add, Text,,       2-15/2-16/2-17			Н/П/К работы по ограбам
    Gui, 5:Add, Text,,       2-20/2-21				Н/К участия в КВГП
    
    WinSet, TransColor, EEAA99 450
    Gui, 5:Show, x5 y3 NoActivate, Window4
}
