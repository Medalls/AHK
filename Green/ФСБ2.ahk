...
ShowStatusMenu() {
    ...
    CurrentMenuLevel := 3
    disableAllHotkeys()
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Выберите статус действия:")
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Нажмите {58e11b}1{FFFFFF} - Начал")
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Нажмите {58e11b}2{FFFFFF} - Продолжаю")
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Нажмите {58e11b}3{FFFFFF} - Закончил")
    addChatMessageEx(-1, "{58e11b}[Диспетчер]{FFFFFF} Нажмите {58e11b}0{FFFFFF} - Отмена")
    SetTimer, DeactivateMenu, -15000
    Hotkey, 1, StatusStart, On
    Hotkey, 2, StatusContinue, On
    Hotkey, 3, StatusEnd, On
    Hotkey, 0, CancelMenu, On
}
...
StatusContinue:
StatusEnd:
    if (!MenuActive || CurrentMenuLevel != 3)
        return
    SetTimer, DeactivateMenu, Off
    MenuActive := false
    CurrentMenuLevel := 0
    ...
    Status := ""
    if (A_ThisLabel = "StatusStart")
        Status := "начал"
    else if (A_ThisLabel = "StatusContinue")
        Status := "продолжаю"
    else if (A_ThisLabel = "StatusEnd")
        Status := "закончил"
    if (SelectedReportType = "прослушка") {
        sendChat("/r Докладывает " . Teg . ": " . Status . " прослушивание организации «" . SelectedOrganization . "».")
        TakeScreenshot()
    } else if (SelectedReportType = "надзор") {
        sendChat("/r Докладывает " . Teg . ": " . Status . " проводить надзорную деятельность за организацией «" . SelectedOrganization . "».")
        TakeScreenshot()
    } else if (SelectedReportType = "внедрение") {
        sendChat("/r Докладывает " . Teg . ": " . Status . " внедрение в организацию «" . SelectedOrganization . "».")
        TakeScreenshot()
    }
return
...
disableAllHotkeys() {
    ...
    if (!IsObject(keys)) {
        keys := []
        keys.Push("0"), keys.Push("1"), keys.Push("2"), keys.Push("3"), keys.Push("4")
        keys.Push("5"), keys.Push("6"), keys.Push("7"), keys.Push("8"), keys.Push("9")
        keys.Push("LAlt & 1"), keys.Push("RAlt & 1")
        keys.Push("LAlt & 2"), keys.Push("RAlt & 2")
    }
    for i, key in keys
